...
JENKINS_NODE_COOKIE=dontKillMe && nohup java -jar $JAR_FILE_NAME --spring.profiles.active=$ACTVIE_PROFILE --server.port=$START_PORT --spring.datasource.url=$DB_URL --spring.datasource.username=$DB_USERNAME --spring.datasource.username=$DB_PASSWORD --jwt.secret-key=$JWT_SECREY_KEY &
