...
JENKINS_NODE_COOKIE=dontKillMe && sudo nohup java -jar $JAR_FILE_NAME --spring.profiles.active=$ACTIVE_PROFILE --server.port=$START_PORT --spring.datasource.url=$DB_URL --spring.datasource.username=$DB_USERNAME --spring.datasource.password=$DB_PASSWORD --jwt.secret-key=$JWT_SECREY_KEY 1> $DEPLOY_PATH/output.log 2>&1 &
